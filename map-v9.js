/* ============================================================
   WAR DESK v9.0 — Conflictkaart
   - v9: NIEUW — "Toon op kaart" knop in event-detail
   - v8: Kaart opent NIET meer automatisch in fullscreen
   - v8: Retry-mechanisme voor 502 fouten
...
(function(){
  ...
  /* ============================================================
     v9: NIEUW — Toon event op de kaart
     ============================================================ */
  function showEventOnMap(event){
    ...
    /* Scroll naar kaart zodat gebruiker het ziet */
    ...
    setTimeout(function(){
      if(targetMarker && typeof MAP.cluster.zoomToShowLayer === "function"){
        /* zoomToShowLayer klapt cluster open tot marker zichtbaar is */
        try{
        ...
        /* Fallback: gewoon naar locatie vliegen */
        MAP.instance.setView([event.lat, event.lng], 10);
      }
    }, 400);
  }
  ...
  async function fetchEvents(retryCount){
    ...
    LOG("Fetch events... (poging " + (retryCount + 1) + ")");
    ...
    for(var i = 0; i < proxies.length; i++){
      ...
      try{
        ...
        var r = await fetch(proxies[i] + encodeURIComponent(apiUrl), {signal: ctrl.signal});
        clearTimeout(timer);
        if(!r.ok) throw new Error("HTTP " + r.status);
        var data = await r.json();
