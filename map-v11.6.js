/* ============================================================
   WAR DESK v11.6 — Conflictkaart (met volledige debug-log)
   - v11.6: Logt naar wdLog voor zichtbaarheid in debug-paneel
   - v11.5: Direct-first fetch
   ============================================================ */
...
(function(){
  ...
  /* ── Log naar zowel console als debug-paneel ── */
  var LOG = function(){
  ...
  LOG("v11.6 geladen — Direct-first modus + wdLog");
  ...
  function openDetail(event){
    ...
    $("wdDetailMeta").textContent = (event.country || "Onbekend") + " · " + timeAgo(event.date) + " · " + (event.confidence || "LOW");
  ...
  async function loadFullText(event, textEl){
    ...
    try{
      ...
      var r = await fetch(MAP.worker + encodeURIComponent(detailUrl));
      if(!r.ok) throw new Error("HTTP " + r.status);
      var data = await r.json();
      var fullText = data.description || event.fullDescription || event.title || "(geen beschrijving)";
      if(Array.isArray(data.article_paragraphs) && data.article_paragraphs.length){
        fullText = data.article_paragraphs.join("\n\n");
      }
  ...
  function showEventOnMap(event){
    ...
    setTimeout(function(){
      if(MAP.instance) MAP.instance.setView([event.lat, event.lng], 8);
    }, 300);
  }

  /* ============================================================
     FETCH EVENTS — Direct first, dan proxies
     ============================================================ */
  async function fetchEvents(retryCount){
    ...
    for(var i = 0; i < proxies.length; i++){
      ...
      try{
        ...
        clearTimeout(timer2);
        LOG("Proxy " + (i+1) + " status: " + r2.status);
    ...
    LOG("❌ Fetch definitief gefaald");
    if(list && !MAP.events.length){
      list.innerHTML = '<div class="live-empty">Kon events niet laden.</div>';
    }
    if(window.showToast) window.showToast("Kon events niet laden.");
  }
  ...
  function processEvents(events){
    ...
    LOG(withCoords.length + " met coords");
  ...
  function renderMarkers(){
    ...
    filtered.forEach(function(e){
      ...
      var popupHtml =
        ...
        '<div class="pop-title">' + escapeHtml(shortDesc) + '</div>' +
  ...
  window.__mapRefresh = function(){ LOG("Handmatige refresh"); fetchEvents(); };
  ...
  console.log("[WAR DESK] map-v11.6.js geladen");
})();