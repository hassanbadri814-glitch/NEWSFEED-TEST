/* ============================================================
   WAR DESK v11.5 — Conflictkaart (Direct-first + Proxy fallback)
   - FIX: Direct fetch EERST (werkte eerder goed)
   - Proxy fallback als direct faalt
   - Retry-mechanisme
   ============================================================ */
...
(function(){
  ...
  LOG("v11.5 geladen — Direct-first modus");
  ...
  function injectMapStyles(){
    ...
    s.textContent =
      ...
      ".wd-detail-foot{border-top:1px solid rgba(255,255,255,.05)!important;background:rgba(0,0,0,.2)!important;display:flex!important;gap:.5rem!important;padding:.85rem 1rem!important}" +
      "body.light .wd-detail-foot{background:rgba(0,0,0,.03)!important}" +
      ".wd-detail-btn{flex:1;background:transparent!important;border:1px solid rgba(255,255,255,.08)!important;color:#8a94a8!important;font-weight:600!important;padding:.75rem 1rem!important;border-radius:10px!important;font-family:inherit!important;font-size:.82rem!important;cursor:pointer!important;transition:all .15s!important;}" +
      ...
      ".wd-detail-btn .ic-btn{width:16px;height:16px;display:inline-block;vertical-align:-3px;margin-right:.4rem;}";
    ...
    modal.innerHTML =
      '<div class="wd-detail-box">' +
        ...
        '<div class="wd-detail-foot">' +
          '<button class="wd-detail-btn primary" id="wdDetailShowOnMap" aria-label="Toon op kaart">' +
            '<svg class="ic-btn" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>' +
            'Toon op kaart' +
          '</button>' +
  ...
  async function loadFullText(event, textEl){
    ...
    try{
      ...
      textEl.style.opacity = "1";
      if(window.showToast) window.showToast("Kon volledige tekst niet laden.");
    }
  }
  ...
  function showEventOnMap(event){
    ...
    if(mapWrap){
      setTimeout(function(){
        try{ mapWrap.scrollIntoView({ behavior: "smooth", block: "start" }); }catch(e){}
      }, 200);
    }

    var targetMarker = null;
    if(MAP.cluster && typeof MAP.cluster.eachLayer === "function"){
      MAP.cluster.eachLayer(function(layer){
        if(targetMarker) return;
        if(layer && typeof layer.getLatLng === "function"){
          var ll = layer.getLatLng();
          if(Math.abs(ll.lat - event.lat) < 0.0001 && Math.abs(ll.lng - event.lng) < 0.0001){
            targetMarker = layer;
          }
        }
      });
    ...
    setTimeout(function(){
      if(targetMarker && typeof MAP.cluster.zoomToShowLayer === "function"){
        try{
          MAP.cluster.zoomToShowLayer(targetMarker, function(){
            if(!MAP.instance) return;
            var z = MAP.instance.getZoom();
            if(z < 10) MAP.instance.setZoom(10);
            MAP.instance.panTo([event.lat, event.lng]);
            setTimeout(function(){
              try{ targetMarker.openPopup(); }catch(e){}
            }, 300);
          });
        }catch(e){
          MAP.instance.setView([event.lat, event.lng], 10);
        }
      } else {
        MAP.instance.setView([event.lat, event.lng], 10);
      }
  ...
  /* ============================================================
     v11.5: FETCH EVENTS — direct first, dan proxies
     ============================================================ */
  async function fetchEvents(retryCount){
    retryCount = typeof retryCount === "number" ? retryCount : 0;
    LOG("fetchEvents poging " + (retryCount + 1) + "/3");
    ...
    /* STAP 1: Direct proberen (zonder proxy) */
    LOG("Probeer DIRECT (zonder proxy)...");
    try{
    ...
    /* STAP 2: Proxies proberen */
    var proxies = [];
    if(window.CONFIG && CONFIG.proxies && CONFIG.proxies.length){
      proxies = CONFIG.proxies.slice();
    }
    if(MAP.worker && proxies.indexOf(MAP.worker) === -1){
      proxies.push(MAP.worker);
    }

    LOG("Probeer " + proxies.length + " proxies...");
    var lastErr = null;

    for(var i = 0; i < proxies.length; i++){
      if(document.hidden && retryCount > 0) return;
      var proxy = proxies[i];
      try{
        LOG("Proxy " + (i+1) + "/" + proxies.length + ": " + proxy.slice(0, 50) + "...");
        var ctrl2 = new AbortController();
        var timer2 = setTimeout(function(){ ctrl2.abort(); }, 12000);
        var r2 = await fetch(proxy + encodeURIComponent(apiUrl), {signal: ctrl2.signal});
        clearTimeout(timer2);
        if(!r2.ok) throw new Error("HTTP " + r2.status);
        var data2 = await r2.json();
        var events2 = (data2 && data2.events) ? data2.events : (Array.isArray(data2) ? data2 : []);
        LOG("✅ Proxy " + (i+1) + " success: " + events2.length + " events");
    ...
    if(retryCount < 2){
      LOG("🔄 Alles faalde, retry in 1.5s...");
    ...
    if(window.showToast) window.showToast("Kon events niet laden. Probeer opnieuw.");
  }
  ...
  function processEvents(events){
    ...
    renderLegend();
    renderLiveList();
    LOG("✅ " + MAP.events.length + " events gerenderd");
  }
  ...
  console.log("[WAR DESK] map-v11.5.js geladen");
})();