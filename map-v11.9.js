/* ============================================================
   WAR DESK v11.9 — Conflictkaart (Alleen Midden-Oosten)
   - Filtert alleen categorieën uit het Midden-Oosten
   - Nieuwsfeed-based (100% betrouwbaar)
...
(function(){
  ...
  LOG("v11.9 geladen — alleen Midden-Oosten");
  ...
  function buildEventsFromNews(){
    if(!window.State || !State.items || !State.items.length){
      LOG("Geen State.items beschikbaar");
      return [];
    }
    ...
    State.items.forEach(function(it){
      var cat = it.cat || "world";
      if(!LOCATIONS[cat]) return;
      if(!byCat[cat]) byCat[cat] = [];
      byCat[cat].push(it);
    });

    LOG("Midden-Oosten categorieën: " + Object.keys(byCat).join(", "));
    ...
    Object.keys(byCat).forEach(function(cat){
      ...
      var titles = top.map(function(it){ return "• " + (it.title || "?"); }).join("\n\n");

      events.push({
        id: "news-" + cat,
        lat: loc.lat,
        lng: loc.lng,
        title: items.length + " artikelen · " + loc.country,
        fullDescription: "Categorie: " + cat + "\nLand: " + loc.country + "\nAantal: " + items.length + " artikelen\n\nLaatste berichten:\n\n" + titles,
        type: cat,
        typeConfig: typeConfig,
        country: loc.country,
        date: top[0].date || new Date().toISOString(),
        url: top[0].link || "",
        source: top[0].source || "",
        confidence: "HIGH",
        count: items.length
  ...
  function refreshFromNews(){
    ...
    if(!events.length){
      LOG("Geen Midden-Oosten events gevonden");
      var list = $("liveList");
  ...
  function initMap(){
    ...
    var mapEl = $("map");
    if(!mapEl) return;
    LOG("Init Leaflet map (Midden-Oosten center)");
  ...
  function startAutoRefresh(){
    ...
    MAP.refreshTimer = setInterval(function(){
      ...
      if(currentCount !== MAP._lastNewsCount){
        MAP._lastNewsCount = currentCount;
        LOG("Nieuwsfeed gewijzigd → kaart updaten");
  ...
  console.log("[WAR DESK] map-v11.9.js geladen");
})();