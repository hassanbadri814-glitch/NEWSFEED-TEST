/* ============================================================
   WAR DESK v11.4 — Conflictkaart (CORS-safe)
   - FIX: Directe fetch verwijderd (CORS-blokkade)
   - Gaat direct door proxies
   - Fallback naar MAP.worker
   - Retry-mechanisme met 1.5s pauze
   ============================================================ */
...
(function(){
  ...
  var LOG = function(){
    ...
    if(window.wdLog && window.wdLog.info) window.wdLog.info(args.join(" "));
  };
  LOG("v11.4 geladen — CORS-safe modus");
  ...
  /* ============================================================
     v11.4: FETCH EVENTS — CORS-safe via proxies
     ============================================================ */
  async function fetchEvents(retryCount){
    ...
    for(var i = 0; i < proxies.length; i++){
      ...
      try{
        ...
        var r = await fetch(proxy + encodeURIComponent(apiUrl), {signal: ctrl.signal});
        clearTimeout(timer);
        if(!r.ok) throw new Error("HTTP " + r.status);
        var data = await r.json();
    ...
    if(retryCount < 2){
      LOG("🔄 Alle proxies faalden, retry in 1.5s...");
  ...
  console.log("[WAR DESK] map-v11.4.js geladen");
})();